...
const UserSuggestions = () => {
  ...
  return (
    <div className="max-w-xs mt-4 bg-white rounded-lg shadow-md p-3">
      ...
      {users.map(({ full_name, role }, index) => (
        <React.Fragment key={index}>
          <div className="flex items-center gap-3 p-2 border-b last:border-none">
            <img src={AvatarDefault} alt={full_name} className="w-10 h-10 rounded-full" />
            <div className="flex-1">
              <h3 className="text-blue-600 text-sm font-medium">{full_name}</h3>
